...
void main() {
    ...
    // Perspective point size: shrink with distance, clamped for sanity.
    float dist = max(v_depth, 0.0001);
    float atten = mix(1.0, (u_viewport_h * 0.5) / dist, u_attenuation);
    gl_PointSize = clamp(u_point_size * atten, 1.0, u_max_point);
...
void main() {
    // Fast path for software rasterisers: a flat opaque square. No discard
    // (which defeats early-Z and forces per-fragment branching), no lighting
    // math, no alpha — the point is just its colour. This is by far the
    // cheapest thing a CPU rasteriser can draw.
    if (u_fast == 1) {
        frag = vec4(v_col, 1.0);
        return;
    }
    ...
    float r2 = dot(pc, pc);
    if (r2 > 1.0) discard;              // round the square sprite

    vec3 col = v_col;
    if (u_shaded == 1) {
        // Reconstruct a spherical normal -> lit surfel.
        vec3 n = vec3(pc.x, -pc.y, sqrt(max(0.0, 1.0 - r2)));
        float diff = max(dot(n, normalize(u_light_dir)), 0.0);
        float ambient = 0.35;
        col = col * (ambient + (1.0 - ambient) * diff);
        // subtle rim to lift dark points off the background
...
class GLViewer(QOpenGLWidget):
    ...
    def set_cloud(self, xyz: np.ndarray, rgb: np.ndarray | None):
        """Replace the displayed cloud. ``xyz`` (N,3); ``rgb`` (N,3) uint8 or None."""
        ...
        inter = np.empty((n, 6), dtype=np.float32)
        inter[:, 0:3] = xyz[perm]
        inter[:, 3:6] = col[perm]
